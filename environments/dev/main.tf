...
module "frontend_vm2" {
  ...
  source              = "../../modules/06_virtual_machine"
  vm_name             = "missionjobvm2"
  resource_group_name = var.resource_group_name
  location            = var.location
  admin_username      = data.azurerm_key_vault_secret.kv-username.value
  admin_password      = data.azurerm_key_vault_secret.kv-password.value
  nic_name            = "missionjob_nic2"
  subnet_id           = data.azurerm_subnet.subdata.id
  nsg_name            = "missionjob_nsg"
}
...
module "sql" {
  ...
  resource_group_name = var.resource_group_name
  location            = var.location
  administrator_login  = data.azurerm_key_vault_secret.kv-username.value
  administrator_login_password  = data.azurerm_key_vault_secret.kv-password.value
  sql_database_name   = "missionjobdb"
